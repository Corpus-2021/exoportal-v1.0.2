...
public class ValidGroupValidator implements Validator {
  public void validate(FacesContext context,  UIComponent component, 
    ...
    if(s == null || s.length() == 0) {
      ...
      throw new ValidatorException(new ExoFacesMessage("#{ValidGroupValidator.msg.empty-input}", args)) ;
    }
    PortalContainer container = PortalContainer.getInstance() ;
    OrganizationService service =  
      (OrganizationService) container.getComponentInstanceOfType(OrganizationService.class) ;
    try {
      if(service.findGroupById(s) == null)  {
        UIInput uiInput = (UIInput) component ;
        Object[] args = {uiInput.getName(), s} ;
        throw new ValidatorException(new ExoFacesMessage("#{ValidGroupValidator.msg.invalid-group-id}", args)) ;
      }
    } catch(Exception ex) {
